...
5. **中性**：避免煽动词（可能大涨、或将暴跌等）
6. **引号铁律（必守）**：JSON 字符串值内部严禁出现 ASCII 双引号 `"`——Claude 最常在这里炸 JSON。中文引用一律用直角引号「」，英文术语若要强调用单引号 `'` 或直接不加引号。例如 `"text": "所谓「AI泡沫论」其实是买入窗口"`，而不是 `"text": "所谓"AI泡沫论"其实是买入窗口"`。违反这一条整条 JSON 会解析失败
